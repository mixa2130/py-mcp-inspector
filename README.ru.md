...
**Скрипт для токена.** В блоке Authentication можно указать путь к своему скрипту: кнопка
**Get token** запускает его и кладёт напечатанное в поле Token, а Connect запускает его перед
подключением. Контракт, примеры и то, что видно в журнале, — в разделе
[Скрипты для токена](#скрипты-для-токена) ниже.

**Обновление токена без реконнекта.** Токен живёт по расписанию сервера, а не сессии, и
переподключение ради свежего токена выбрасывает и хендшейк, и `mcp-session-id`, и всё, что за
ним числится. Поэтому изменённый токен применяется сам: поправьте Token, Scheme или имя
заголовка на живой HTTP-сессии — и, как только поле затихнет, инспектор подменит учётку на тех
HTTP-клиентах, которые транспорт уже использует. httpx подмешивает `client.headers` в каждый
запрос в момент его сборки, поэтому новое значение уходит начиная со следующего запроса, а
сессия остаётся та же. Отдельной кнопки для этого нет: токен, который лежит в форме, но не
уходит в сеть, — ровно та путаница, ради которой инспектор и написан.

Пустой токен просто перестаёт отправляться, а смена имени заголовка снимает старое имя, чтобы
два варианта не уходили рядом. На простаивающем инспекторе не уходит ничего — поле там просто
учётка для следующего Connect, — а значение, которое у сессии уже есть, не стоит и запроса.
Что именно произошло, видно в журнале: `Authorization replaced (20 chars); in effect from the
next request`.

Что важно помнить: SSE-стрим, открытый раньше, был аутентифицирован при открытии и продолжает
жить со старым токеном — продлевать его или нет, решает сервер. Об этом пишется строка в Log.
Для stdio не применяется ничего: окружение дочернего процесса фиксируется при запуске.

Из скрипта — одним запросом, например по таймеру рядом с выдающим токен сервисом:
...
**TLS.** Отдельный блок в сайдбаре для HTTP-транспортов: свой CA, клиентский сертификат для
mTLS, пароль к зашифрованному ключу и галка, отключающая проверку целиком. Поля, ошибки, которые
они дают, и одноразовый CA, на котором всё это можно попробовать, — в разделе
[TLS и mTLS](#tls-и-mtls) ниже.
...
Он же умеет HTTPS и может требовать клиентский сертификат — так блок TLS проверяется целиком,
см. [TLS и mTLS](#tls-и-mtls) ниже.

## TLS и mTLS

Блок TLS появляется в сайдбаре для `sse` и `streamable-http`. Он, как и **Advanced**, свёрнут по
умолчанию — щелчок по заголовку (или Enter/Space с клавиатуры) разворачивает его, и выбор
запоминается в браузере. Пока секция свёрнута, в её заголовке висит чип с тем, что внутри
...
Пути проверяются до подключения, поэтому отсутствующий файл или неверный пароль так и говорят,
а не отваливаются таймаутом где-то в хендшейке:
...
Сертификат, которого не знает хранилище доверия, падает как положено, называя недоверенный
центр: `ConnectError: ('"PyMCPinspector Demo CA" certificate is not trusted',)`. После
подключения действующий набор перечислен в панели Server — в том числе то, что ключ зашифрован:
на вопрос «каким сертификатом ходила эта сессия» надо уметь ответить, не перечитывая конфиг.
...
В пресетах лежат пути, а не сами сертификаты, так что переданный пресет указывает на файлы,
которые должны быть у получателя. Пароль к ключу хранится в файле пресетов открытым текстом —
и вычищается при экспорте, как любой другой секрет.

### Как попробовать на одноразовом CA

Демо-сервер говорит по HTTPS и умеет требовать клиентский сертификат, так что весь путь
проверяется, не трогая ничего настоящего. Выпускаем CA и два подписанных им сертификата:
...
openssl rsa -in client.key -aes256 -passout pass:hunter2 -out client-enc.key   # чтобы задействовать поле пароля
cd ..
```

Две строки `-addext` у CA — не украшение: OpenSSL 3 отказывается проверять цепочку, у чьего CA
нет `keyCertSign`, и ошибка выглядит как `CA cert does not include key usage extension`, а не
как подсказка, что расширение надо было добавить самому.

Дальше запускаем сервер с обязательным mTLS и указываем инспектору:
...
Connect. Самый поучительный отказ — убрать поле CA: сертификат сервера совершенно валиден,
просто системное хранилище никогда не слышало о подписавшем его центре. Снятая галка
**Verify the server certificate** это обходит — она же добавляет `--insecure` в собранную
команду `curl` и `no verify` в чип свёрнутой секции.

## Скрипты для токена

OAuth-флоу в инспекторе нет и не планируется: токены везде выдают по-своему. Вместо него есть
крючок — ваш скрипт, который печатает учётку, а инспектор его запускает и берёт напечатанное.
Укажите **Token script** (и **Script arguments**) в блоке Authentication, и дальше:

- **Get token** запускает скрипт и кладёт результат в поле Token — на живой сессии сразу
  применяя его;
...
имя заголовка сменилось само.

### Контракт

Инспектор не знает о скрипте ничего, кроме пути, поэтому контракт тонкий:

| Что | Как |
|---|---|
| аргументы | из поля **Script arguments**, разбираются как в оболочке |
| контекст | `PYMCPINSPECTOR_URL`, `PYMCPINSPECTOR_TRANSPORT`, `PYMCPINSPECTOR_AUTH_HEADER`, `PYMCPINSPECTOR_AUTH_SCHEME` плюс всё окружение инспектора |
...
Самое короткое, что ему удовлетворяет:

```python
#!/usr/bin/env python3
import os, sys, urllib.request, json

print(f"minting for {os.environ['PYMCPINSPECTOR_URL']}", file=sys.stderr)   # уйдёт в журнал
print(json.load(urllib.request.urlopen(os.environ["TOKEN_ENDPOINT"]))["access_token"])
```

Объектная форма — для сервера, которому учётка нужна совсем в другом месте: не bearer-токен,
а ключ API:
...
Инспектор тогда переставит поля Scheme и Header name, чтобы сайдбар по-прежнему показывал то,
что реально уходит. Незнакомые ключи игнорируются, и о том, какие именно, пишется в журнал.

### Запуск и что видно в журнале

Скрипт запускается **отдельным процессом**, а не импортируется: скрипту обычно нужны
библиотеки, которых нет в окружении инспектора, зависший процесс надо уметь убить (таймаут —
60 секунд), а упавший не должен ронять инспектор. Исполняемый файл запускается сам собой — так
шебанг может указывать на нужный venv; всё остальное отдаётся тому же интерпретатору, на
котором работает инспектор.

В журнал уходит точная команда, каждая строка stderr и то, что инспектор решил за скрипт: какую
строку stdout счёл токеном, какие ключи JSON проигнорировал. Сам токен в журнал не попадает —
только его длина:
...
Текущий токен скрипту **не передаётся**: тому, кто печатает новый, старый не нужен. Права у
скрипта те же, что у инспектора, — это тот же уровень доверия, что и у `command` в
...
[examples/token_plugin.py](examples/token_plugin.py) проходит все ветки контракта и работает без
сети — с него удобно начать:

```bash
python examples/demo_server.py --transport streamable-http --port 8931
pymcpinspector
# в UI: Streamable HTTP  -> http://127.0.0.1:8931/mcp
#       Token script     -> examples/token_plugin.py
#       Script arguments -> --profile prod
#       Connect -> Get token -> Tools -> show_headers -> Run tool
```

`show_headers` вернёт тот самый заголовок, который выдал скрипт, — это и есть доказательство,
что цикл замкнулся. Аргументы скрипта — это его ветки: `--profile ИМЯ` выбирает, что чеканить,
`--json` отвечает объектной формой (и переносит учётку в `X-Api-Key`), `--fail` завершается
ненулевым кодом, чтобы увидеть, как выглядит провал: поле Token остаётся нетронутым, а причина
показывается.

[examples/oauth_token_plugin.py](examples/oauth_token_plugin.py) показывает форму, из-за которой
токен и приходится добывать скриптом: пароль меняется на токен пользователя, тот — на токен
агента для нужной audience, а заголовок с идентификатором сотрудника задаётся аргументом.
Секретов в нём нет, они берутся из окружения инспектора:
...
| POST | `/api/auth/plugin` | тело — конфигурация подключения; запускает её скрипт токена → `{token, scheme, header, notes}` |
...
- Встроенного OAuth-флоу нет: токен либо вводится руками в блоке аутентификации, либо печатается
  [скриптом](#скрипты-для-токена). Правка долетает до живой сессии сама, но истечение токена
  инспектор не отслеживает: без нажатия **Get token** или своего запроса к `/api/auth` ничего
  не перевыпускается.
